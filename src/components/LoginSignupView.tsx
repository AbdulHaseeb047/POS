  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
        const { email: gEmail, name: gName } = event.data.payload || {};
        if (gEmail) {
          const staffUser = {
            id: `google-${Date.now()}`,
          ...
          loginUser(staffUser);
          setSuccessMsg(`Successfully authenticated via Google as ${gName || gEmail}!`);
        }
      }
    };
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      const authWindow = window.open(
        url,
        'google_oauth_popup',
        'width=500,height=620,status=no,resizable=yes,scrollbars=yes'
      );
      
      if (!authWindow) {
        setError('Popup was blocked! Please enable popups in your browser to sign in with Google.');
      }
    } catch (err: any) {
          ...
          {isLogin ? (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-4 text-slate-500 text-[10px] uppercase font-mono tracking-wider">or verify with</span>
                <div className="flex-grow border-t border-slate-700/50"></div>
              </div>
              ...
              <button
                ...
                className="w-full bg-white hover:bg-slate-100 text-slate-800 font-bold py-2.5 px-4 rounded-xl text-xs transition-all flex items-center justify-center gap-2.5 shadow-md border border-slate-200 cursor-pointer"
              >
                ...
                <span>Google Verification Sign In</span>
              </button>
            ...
            <form onSubmit={handleSignupSubmit} className="space-y-4">
              ...
              <div className="relative flex py-2 items-center">
                <div className="flex-grow border-t border-slate-700/50"></div>
                <span className="flex-shrink mx-4 text-slate-500 text-[10px] uppercase font-mono tracking-wider">or register with</span>
                <div className="flex-grow border-t border-slate-700/50"></div>
              </div>
